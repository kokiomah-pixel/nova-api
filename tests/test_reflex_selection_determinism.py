...
def test_reflex_selection_is_deterministic_for_same_regime_and_intent():
    ...
    assert [entry.registry_id for entry in selected if entry] == ["elevated_fragility_constraint_context"] * 5
...
def test_reflex_selection_uses_canonical_priority_when_multiple_entries_apply():
    ...
    assert selected.registry_id == "stress_prior_stress_review_attention"
