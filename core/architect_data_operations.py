...
def render_architect_brief(snapshot: Mapping[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Canonical source state: {sync.get('Registry_ingestion', {}).get('canonical_source_state') or 'unknown'}",
            f"- Checkout commit: {sync.get('Registry_ingestion', {}).get('checkout_commit') or 'unknown'}",
            f"- Canonical main commit: {sync.get('Registry_ingestion', {}).get('canonical_main_commit') or 'unknown'}",
            f"- Checkout is canonical main: {str(sync.get('Registry_ingestion', {}).get('checkout_is_canonical_main', False)).lower()}",
            f"- Canonical main verified: {str(sync.get('Registry_ingestion', {}).get('canonical_main_verified', False)).lower()}",
            f"- Canonical registry loaded: {str(sync.get('Registry_ingestion', {}).get('canonical_registry_loaded', False)).lower()}",
            f"- Canonical source: {sync.get('Registry_ingestion', {}).get('canonical_source') or 'unknown'}",
            f"- Canonical commit: {sync.get('Registry_ingestion', {}).get('canonical_commit') or 'unknown'}",
            f"- Current accepted-state claim allowed: {str(sync.get('Registry_ingestion', {}).get('current_accepted_state_claim_allowed', False)).lower()}",
            f"- Checkpoint status: {sync.get('Registry_ingestion', {}).get('checkpoint_status') or 'unknown'}",
            f"- Mirror status: {sync.get('Registry_ingestion', {}).get('mirror_status') or 'unknown'}",
            f"- Newly accepted: {sync.get('Accepted_state_delta', {}).get('newly_accepted') or 'none'}",
            f"- Stable accepted state: {sync.get('Accepted_state_delta', {}).get('stable_accepted_state') or []}",
